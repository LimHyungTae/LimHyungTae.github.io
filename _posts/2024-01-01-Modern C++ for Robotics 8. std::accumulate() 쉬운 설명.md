...
std::accumulate를 의도대로(?) 사용하면 평균을 구하는 함수도 한 줄로 작성이 가능하다:
...
하지만 위의 코드는 파일을 불러오는 코드여서 상관없긴 하지만, std::accumulate를 활용해서 각 요소를 비교하면, `normals_indicator`의 모든 요소가 조건을 충족하는지 확인해야 한다는 단점이 있다.
