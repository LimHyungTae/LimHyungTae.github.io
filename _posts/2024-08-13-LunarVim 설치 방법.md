...
tags: [Ubuntu, vim]
comments: true
description: Ubuntu에 LunarVim을 새로 설치하기 위한 NeoVim 0.10.0, vim-plug, NodeJS 버전 업데이트, LunarVim 설치 스크립트 실행, ROS launch 파일 syntax highlight 설정까지 단계별로 정리한다.
permalink: /2024/08/13/lunarvim-installation/
redirect_from:
  - '/2024-08-13-LunarVim 설치 방법/'
  - '/2024-08-13-LunarVim-설치-방법/'
---

현재 연구실에서 개발해야 하는 환경이 Vim을 안 쓰고서는 도저히 할 수 없는 구조여서, 다시 LunarVIM으로 회귀해본다.

LunarVim을 깔기 이전에 이것저것 깔아야 하는데, 이 과정을 까먹지 않기 위해 기록으로 남긴다 (늘 NeoVim깔 때 버전 문제가 있었기 때문....).
...
이는 아래와 같이 간단히 해결할 수 있다 (현재 내 컴퓨터에서는 npm이 깔려 있었다):
...
기본적으로 LunarVim으로 ROS의 launch 파일을 열면 highlight가 되지 않는데, `config.lua`에 다음과 같이 두 개 추가하니 해결이 되었다.
