...
void labelComponents(int row, int col){
    ...
    while(queueSize > 0){
        ...
        for (auto iter = neighborIterator.begin(); iter != neighborIterator.end(); ++iter){
            ...
            // extern const float segmentAlphaX = ang_res_x / 180.0 * M_PI; (X축, 즉 rowIdn 방향)
            // extern const float segmentAlphaY = ang_res_y / 180.0 * M_PI; (Y축, 즉 columnIdn 방향)로 미리 정의된 segmentAlpha 사용
            // 근데 아래 if 조건문이 반대로 된 거 같은데...? dx = 0이면 segmentAlphaY가 들어가야 하는 거 아닌가????
            if ((*iter).first == 0)
                alpha = segmentAlphaX;
            else
                alpha = segmentAlphaY;
