...
comments: true
description: ROS와 함께 깔린 OpenCV 3.2를 깔끔히 제거하고 CUDA, CuDNN, Ceres Solver 의존성과 함께 OpenCV 3.4를 빌드한 뒤 cv-bridge까지 재설치하는 r3live 환경 세팅 과정을 기록한다.
permalink: /2022/08/17/ubuntu-1804-opencv-32-to-34-upgrade/
redirect_from:
  - '/2022-08-17-Ubuntu 18.04에서 OpenCV 3.2에서 OpenCV 3.4로 업그레이드 하기/'
  - '/2022-08-17-Ubuntu-18.04에서-OpenCV-3.2에서-OpenCV-3.4로-업그레이드-하기/'
---
...
처음에는 `CUDA_nppicom_LIBRARY not found` 에러가 났는데, 해당 문서에는 `sudo-apt get install nvidia-cuda-toolkit`을 치거나 `opencv/cmake/OpenCVDetectCUDA.cmake`를 수정하라고 되어 있는데, 예전에 toolkit 설치하다가 CUDA 관련 이슈로 포맷을 한 적이 있어서 후자를 택했다 (and it works).
...
그 후, ROS cv-bridge를 다시 설치해주어야 한다. [여기](https://github.com/engcang/vins-application#-ceres-solver-and-eigen-mandatory-for-vins)에 따르면, OpenCV를 수동으로 설치했으면 package로 빌드해야 한다고 한다 (하지만 테스트 해보진 않음).
...
하니 해결됐다. 설치할 때 명령어 차이라고는 원래는 스킵하던 `make test`를 한 것밖에 없는데...흠...찜찜하지만 무사해결됐으니...

### 그 후 ROS package를 빌드하면

아마 기존의 많은 library들이 없어져 있을 것이다. `sudo apt autoremove`로 대다수의 이미지 관련 패키지들이 지워졌기 때문이다.
(pcl-ros도 지워짐)
