...
comments: true
description: 두 keyframe 사이 IMU 데이터를 하나의 factor로 압축하는 preintegrated relative motion increments(rotation, velocity, position)와 bias 업데이트 수식을 차례로 유도하고 GTSAM 구현과 연결한다.
image: /img/preintegration/final_i_j_v2.png
...
![keyframe i와 j 사이 motion 적분 수식](/img/preintegration/final_i_j_v2.png)
...
이러한 문제를 해결하기 위해 논문의 저자는 수백 여개의 measurements를 factor로 직접적으로 넣는 것이 아니라, factor graph에 measurement를 추가하기 이전에 (pre-) 수백여개의 IMU data를 단 하나의 factor로 취합(integration)하는 방법에 대해서 제안한다.
...
![relative motion increments 정의](/img/preintegration/relative_motion_increments.png)
...
![물리적 relative motion 의미](/img/preintegration/physical_meaning.png)


**Assumption**: 그 후, 수식을 전개하기 전 $$i$$ 번째와 $$j$$ keyframe 사이에서는 bias가 일정하다고 가정한다.

![preintegration의 bias 가정](/img/preintegration/preint_bias.png)
...
![preintegrated rotation 유도](/img/preintegration/preinteg_rot_v2.png)

### Preintegrated Velocity Measurements
![preintegrated velocity 유도](/img/preintegration/preinteg_vel_v2.png)

### Preintegrated Position Measurements

![preintegrated position 유도](/img/preintegration/preinteg_pos_v2.png)


## Do the Uncertainties behave like Gaussian distribution?

위의 전개를 하다 보면 '이걸 왜 하고 있는 거지...?'하며 정신이 살짝 아득해지는데, 이제 거의 다 왔다! 위의 전개를 통해 증명하고자 하는 것은 "IMU data로 measured된 relative motion increments는 추정해야 하는 (to-be-estimated) relative motion increments과 gaussian distribution을 따르는 random vector의 합의 꼴로 표현이 가능하다"는 것이다.
...
![Gaussian 분포로 근사 수식](/img/preintegration/gaussian_distribution.png)
...
![relative motion increments의 Gaussian 모델링](/img/preintegration/behave_like.png)
...
![bias residual term 정의](/img/preintegration/bias_description.png)
...
```
 ...
 그리고 relative motion increments에 대한 세 term과 biases term 또한 `gtsam::ImuFactor`라는 클래스에 preintegrated measurements를 대입해주면 알아서 계산하게 이미 잘 구현되어 있다. 아래는 LIO-SAM의 `imuPreintegration.cpp` 내부의 코드의 예제이다.
...
따라서 implementation 관점에서 preintegration을 하기 위해 기억해야 할 것을 정리하자면 아래와 같다.
...
더 자세한 사항은 [다음 글](https://limhyungtae.github.io/2022-04-01-IMU-Preintegration-(Easy)-5.-IMUPreintegration-in-LIO-SAM/)에서 코드를 line-by-line으로 살펴보며 설명한다.
