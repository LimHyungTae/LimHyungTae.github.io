...
이 시리즈를 처음 작성한 게 무려 2019년이다. 1편에서는 Ceres Solver의 hello world와 cost function 등록 방법 정도만 다뤘는데, 그 이후로 글이 멈춰있었다. ~~6년 동안 어디 갔다왔지?~~ 사실 그 사이에 IMU Preintegration 시리즈와 GTSAM Tutorial 시리즈를 작성하면서 "Ceres도 한 번은 정리해야 하지 않나"라는 생각을 계속 가지고 있었는데, 마침 GTSAM 시리즈를 어느 정도 마무리한 차에 다시 글을 이어가게 되었다.

이번 sub-series에서는 Ceres가 공식 example로 제공하는 [Pose Graph 3D 예제](https://github.com/ceres-solver/ceres-solver/tree/master/examples/slam/pose_graph_3d)를 line-by-line으로 깊게 분석할 것이다. 이 예제는 SLAM에서 가장 기본이 되는 **pose graph optimization** 문제를 정확히 푸는 코드인데, 코드 자체는 200줄도 안 되는 두 파일 ([`pose_graph_3d.cc`](https://github.com/ceres-solver/ceres-solver/blob/master/examples/slam/pose_graph_3d/pose_graph_3d.cc), [`pose_graph_3d_error_term.h`](https://github.com/ceres-solver/ceres-solver/blob/master/examples/slam/pose_graph_3d/pose_graph_3d_error_term.h)) 이지만, Ceres가 SLAM optimization을 푸는 방식의 모든 핵심이 이 안에 들어있다.
...
다음 [4편](https://limhyungtae.github.io/2025-05-07-Ceres-Solver-for-Graph-SLAM-4.-PoseGraph3dErrorTerm-%EA%B9%8A%EA%B2%8C-%EC%9D%B4%ED%95%B4%ED%95%98%EA%B8%B0/)에서는 `pose_graph_3d_error_term.h`를 한 줄 한 줄 뜯어보면서, 6차원 residual이 수학적으로 정확히 무엇을 의미하는지, 그리고 `template <typename T>` 한 줄이 어떻게 AutoDiff로 Jacobian을 만들어주는지를 깊게 다룰 것이다.
