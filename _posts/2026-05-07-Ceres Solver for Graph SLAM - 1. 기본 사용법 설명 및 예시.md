...
튜토리얼 자료는 [LimHyungTae/helloceres](https://github.com/LimHyungTae/helloceres) 레포지토리에 올려두었다. 이 글에서 다룰 내용에 대한 minimal example들이 들어있다.
...
Source build가 필요한 경우, 예를 들어 구체적인 version pinning이나 옵션 조정이 필요할 때는 Ceres 공식 [installation 가이드](http://ceres-solver.org/installation.html)를 참고하면 된다.
...
손으로 풀어봐도 간단하다. gradient를 0으로 두면 $$x = (10 + 5) / 2 = 7.5$$가 답이다. 정확히 두 측정값의 평균이다. 코드는 다음과 같다.
