...
Range image를 만든 후에, range image를 활용하여 ground points를 labeling한다. 이 과정에서 암묵적으로 3D LiDAR sensor가 수평하게 모바일 플랫폼에 부착되어 있다고 가정한다. 그렇기 때문에 16 channel LiDAR의 경우 1~8번째 channel의 경우에만 땅바닥쪽으로 laser ray가 방출되고 있기 때문에, `groundScanInd=7`로 세팅한 것을 볼 수 있다.
...
전체 코드는 아래와 같고, 중요한 부분을 step-by-step으로 설명한다. 
