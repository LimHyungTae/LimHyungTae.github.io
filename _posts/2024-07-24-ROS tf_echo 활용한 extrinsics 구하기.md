...
tags: [ROS, C++, RViz, tf]
comments: true
description: ROS의 rosrun tf tf_echo 명령으로 두 frame 사이의 transformation을 출력해, base_link 기준 LiDAR 위치나 LiDAR-IMU extrinsic을 구하는 방법을 예제로 정리한다.
...
라고 입력하면 T_AB를 구할 수 있다. 즉, B frame을 A frame으로 변환하는 행렬(Transformation of B w.r.t. A)이다.
...
이처럼 extrinsic을 구할 때에는, 센서의 설치 방향을 기반으로 출력된 값이 물리적으로 make sense한지 한 번 확인해보는 게 중요하다.

## Example 2

만약 Ouster OS1-64 내부의 LiDAR와 IMU를 이용해서 Fast-LIO2를 돌리려면 어떻게 해야 할까? 아래와 같이 입력하면 된다.
