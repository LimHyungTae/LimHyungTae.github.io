...
comments: true
description: 직접 pybind11 toy 예제를 빌드하면서 .so shared object의 의미와 setup.py가 wheel 파일을 만들어 로컬 어디서나 import 가능하게 해주는 원리를 차근차근 풀어본다.
...
이는 사실 생성된 `.so` 파일을 읽어서 동작하는 것이다. 만약 `.so`를 다른 폴더로 옮기고 파일을 실행시키게 되면, error가 나는 것을 볼 수 있다.
...
![pip install로 wheel 빌드되는 로그](/img/pybinding_cmake_example_results.png)
