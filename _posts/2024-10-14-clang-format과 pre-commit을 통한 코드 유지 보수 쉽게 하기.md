...
comments: true
description: clang-format과 pre-commit hook을 활용해 팀 단위 C++ 코드 스타일을 자동으로 강제하고 일관성을 유지하는 설정 방법과 실전 옵션을 정리한다.
image: /img/pre-commit-example.png
...
![pre-commit cpplint 에러 예시](/img/pre-commit-example.png)
...
특히 나의 경우는, 해당 repository를 더 이상 관리하지 않기 때문에 노력을 많이 쏟고 싶지 않았다.
그래서 고치지 않아도 무방한 부분을 해당 `--filter`에 추가해주었다.
추가하는 방법은, 아래 스크린샷의 `[${CPPLINT TYPE}]`의 `${CPPLINT TYPE}` 부분을 filter 뒤에 추가해주면 된다:

![cpplint filter 옵션 예시](/img/pre-commit-example2.png)
